...
function saveData(trials) {
    let headers = ['blockNo', 'trialNo', 'prop', 'qty', 'startPrice', 'endPrice', 'step', 'oppBid', 'price', 'winner'];
    ...
    let filePath = path.join(__dirname, folderName, fileName + fileExtension);

    saveCSV(headers, data, filePath);
}

function recordBlockData(block, blockNo) {
    for (let userName in block) {
        if (block.hasOwnProperty(userName)) {
            let user = block[userName];

            session.blocks.push([userName, user.goods, user.money, blockNo]);
        }
    }
}

function saveBlockData(blocks) {
    let headers = ['userName', 'warehouseQty', 'fundsRemaining', 'blockNo'];
    let filePath = path.join(__dirname, folderName, fileName + '_BLOCK' + fileExtension);

    saveCSV(headers, blocks, filePath);
}
...
function bid(event) {
    if (session.trial.status === 'running') {
        ...
        if (session.trial.trialNo === conf.nTrials - 1) {
            recordBlockData(session.users, session.trial.blockNo + 1);
...
io.on('connection', function(socket) {
    ...
    socket.emit('init', {
        ...
        fogOfWarehouse: conf.fogOfWarehouse
    });
